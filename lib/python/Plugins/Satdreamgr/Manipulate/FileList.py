...
def FileEntryComponent(name, absolute = None, isDir = False):
    ...
    res.append((eListboxPythonMultiContent.TYPE_TEXT,
     ...
     name))
    if isDir:
        png = LoadPixmap('/usr/lib/enigma2/python/Plugins/Satdreamgr/Manipulate/images/directory.png')
    else:
        extension = name.split('.')
        extension = extension[-1].lower()
        if EXTENSIONS.has_key(extension):
            png = LoadPixmap('/usr/lib/enigma2/python/Plugins/Satdreamgr/Manipulate/images/' + EXTENSIONS[extension] + '.png')
        else:
            png = None
    if png is not None:
